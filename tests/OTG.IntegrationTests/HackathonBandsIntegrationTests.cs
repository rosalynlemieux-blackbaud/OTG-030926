...
public sealed class HackathonBandsIntegrationTests
{
    [Fact]
    public async Task UpdateLeaderboardBands_Succeeds_ForAdmin()
    {
        await using var factory = new TestApiFactory();
        var admin = factory.SeedUser("bands-admin@example.com", UserRole.Admin);
        var hackathon = factory.SeedHackathon("hack-bands-1");

        var client = factory.CreateClient(new WebApplicationFactoryClientOptions { AllowAutoRedirect = false });
        AddAuthHeaders(client, admin.Id, admin.Email, "Admin");

        var response = await client.PutAsJsonAsync($"/api/hackathons/{hackathon.HackathonId}/leaderboard-bands", new
        {
            PlatinumMinPercentile = 92m,
            GoldMinPercentile = 70m,
            SilverMinPercentile = 40m
        });

        Assert.Equal(HttpStatusCode.OK, response.StatusCode);
        var updated = await response.Content.ReadFromJsonAsync<LeaderboardBandSettings>();
        Assert.NotNull(updated);
        Assert.Equal(92m, updated!.PlatinumMinPercentile);
        Assert.Equal(70m, updated.GoldMinPercentile);
        Assert.Equal(40m, updated.SilverMinPercentile);
    }

    [Fact]
    public async Task UpdateLeaderboardBands_ReturnsForbidden_ForParticipant()
    {
        await using var factory = new TestApiFactory();
        var participant = factory.SeedUser("bands-participant@example.com", UserRole.Participant);
        var hackathon = factory.SeedHackathon("hack-bands-2");
        ...
        var response = await client.PutAsJsonAsync($"/api/hackathons/{hackathon.HackathonId}/leaderboard-bands", new
        {
            PlatinumMinPercentile = 90m,
            GoldMinPercentile = 75m,
            SilverMinPercentile = 50m
        });
    ...
    [Fact]
    public async Task UpdateLeaderboardBands_ReturnsBadRequest_ForInvalidThresholdOrdering()
    {
        await using var factory = new TestApiFactory();
        var admin = factory.SeedUser("bands-admin-invalid@example.com", UserRole.Admin);
        var hackathon = factory.SeedHackathon("hack-bands-3");
    ...
    private sealed class TestApiFactory : WebApplicationFactory<OTG.Api.Program>
    {
        ...
        public Hackathon SeedHackathon(string hackathonId)
        {
